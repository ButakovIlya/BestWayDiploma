...
import { isPublicPlace } from "@/app/dashboard/my-routes/types/custom-place";
...
export function ChooseFromPlacesModal() {
  ...
  const { pushCustomPlaces, customPlaces } = createRoute;
  const {
    isAddModalOpened,
    isTableDataLoading,
    setIsAddModalOpened,
  ...
  const chosenCustomPlacesIds = customPlaces
    .filter((place) => isPublicPlace(place))
    .map((place) => place.id);
